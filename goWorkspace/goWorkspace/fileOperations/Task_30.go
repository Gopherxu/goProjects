// to avoid singal letter words and words with high frrequency
...
// function to process word count
func processWordCount(WordCount map[string]int) map[string]int {

	//Creating new map to store processed wordcount
	processedCount := make(map[string]int)

	for word, count := range WordCount {

		// ignore words with single charactor and having count <= 5
		if len(word) > 1 && count < 5 {
...
func main() {
	...
	//It stores less Frequent words and ignores single charactor words
	processedCount := processWordCount(WordCount)

	//Print words and their frequencies
	for word, count := range processedCount {
